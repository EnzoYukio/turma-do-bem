      ...
      <main className="flex-1 max-w-4xl mx-auto w-full px-4 py-6 pb-24 md:pb-6">
        ...
        {!mostrarForm && (
          <div className="mb-4 text-right">
            <button onClick={() => setMostrarForm(true)} className="bg-blue-600 hover:bg-blue-800 text-white font-semibold px-5 py-2 rounded-xl">
              + Novo Paciente
            </button>
          </div>
        )}
        ...
        {mostrarForm && (
          <div className="bg-white rounded-xl shadow p-5 mb-4">
            <h2 className="text-blue-800 font-bold text-lg mb-4">{editando ? 'Editar Paciente' : 'Novo Paciente'}</h2>

            <label className="block text-sm text-gray-500 mb-1">Nome *</label>
            <input type="text" value={form.nome} onChange={(e) => setForm({ ...form, nome: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-blue-300" />

            <label className="block text-sm text-gray-500 mb-1">CPF * (só números, 11 dígitos)</label>
            <input type="text" value={form.cpf} maxLength={11} onChange={(e) => setForm({ ...form, cpf: e.target.value.replace(/\D/g, '') })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-blue-300" />

            <label className="block text-sm text-gray-500 mb-1">Email *</label>
            <input type="email" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-blue-300" />

            <label className="block text-sm text-gray-500 mb-1">Telefone *</label>
            <input type="text" value={form.telefone} onChange={(e) => setForm({ ...form, telefone: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-blue-300" />

            <label className="block text-sm text-gray-500 mb-1">Dentista responsável</label>
            <input type="text" value={form.dentista} onChange={(e) => setForm({ ...form, dentista: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm mb-3 focus:outline-none focus:ring-2 focus:ring-blue-300" />

            <label className="block text-sm text-gray-500 mb-1">Endereço</label>
            <input type="text" value={form.endereco} onChange={(e) => setForm({ ...form, endereco: e.target.value })} className="w-full border border-gray-200 rounded-xl px-4 py-3 text-sm mb-4 focus:outline-none focus:ring-2 focus:ring-blue-300" />

            <div className="flex gap-3">
              <button onClick={enviarForm} className="bg-blue-600 hover:bg-blue-800 text-white font-semibold px-5 py-2 rounded-xl">{editando ? 'Salvar' : 'Adicionar'}</button>
              <button onClick={resetarForm} className="border border-gray-300 text-gray-600 hover:bg-gray-50 font-semibold px-5 py-2 rounded-xl">Cancelar</button>
            </div>
          </div>
        )}
        ...
        <div className="bg-white rounded-xl shadow p-5">
          ...
          {!carregando && pacientes.length === 0 && <p className="text-gray-400 text-sm">Ainda não há pacientes cadastrados.</p>}
          {pacientes.map((p) => (
            <div key={String(p.id)} className="border border-gray-100 rounded-xl p-4 mb-3">
              ...
              <div className="flex gap-2">
                <button onClick={() => prepararEdicao(p)} className="bg-yellow-400 hover:bg-yellow-500 text-white text-sm font-semibold px-4 py-2 rounded-lg">Editar</button>
                <button onClick={() => removerPaciente(p.id)} className="bg-red-500 hover:bg-red-700 text-white text-sm font-semibold px-4 py-2 rounded-lg">Remover</button>
              </div>
            </div>
          ))}
        </div>
